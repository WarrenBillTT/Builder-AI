...
async function runBackgroundGeneration(projectId, prompt){
    try {
        ...
        const result = await generateProject(prompt, {
            onPlan: async (plan) => {
                console.log(`[Background AI] Plan created for projects ${projectId}. Planned ${plan.files.length} files.`);
        ...
        console.log(`[Background AI] Fatal generation error for project ${projectId}:`, err);
        await Project.findByIdAndUpdate(projectId, {
            status: "failed",
            error: err.message,
            $push: {
